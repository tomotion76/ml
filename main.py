...
if train_file:
    # pd.read_csv()
    train = pd.read_csv(train_file)# 코드 입력
...
if train_btn:
    ...
    # 학습 결과 출력
    best_model = compare_models(sort='Accuracy', n_select=3, fold=5)# 코드 입력
    # 모델 블렌딩
    blended_models = blend_models(best_model, fold=5)# 코드 입력
    # 학습 결과 받아오기
    result = pull()
    st.dataframe(result)
    # 모델 저장
...
if test_btn:
    # 모델 로드 load_model()
    loaded_model = load_model('classification') # 코드 입력
    
#     # 예측 predict_model()
    predictions = predict_model(estimator=loaded_model, data=test)# 코드 입력
